...
// registry. `sr_{runId}` keys are one-shot scenario-run harness rooms looked
// up in the ScenarioRunRegistry. The `hub` routing key (empty URL path) is
// handled by the shared `DefaultGameOrchestration` singleton. Unknown keys
