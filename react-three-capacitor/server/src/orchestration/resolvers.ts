...
// registry. The `hub` routing key (empty URL path) is handled directly by
// GameServer — it creates a per-player solo hallway MR and transfers the
// player into an open hub slot on a target scenario MR, so it does NOT
// resolve here. Unknown keys resolve to null; the router rejects those
