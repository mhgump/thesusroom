...
// spans the full inner width (CORR_INNER_W) and the scenario drops it
// when a player uses the OPEN ability near it (see scenario.ts). There is
// no floor-level button geometry — the OPEN affordance lives on the HUD
// ability bar.
...
const INSTRUCTION_SPECS: InstructionEventSpec[] = [
  { id: 'rule_open',     text: 'Press OPEN to unlock the next gate', label: 'RULE' },
...
// Gate ids (ordered south → north): gate1, gate2, gate3.
// Gate z-coordinates (local corridor frame): GATE1_Z, GATE2_Z, GATE3_Z.
// The scenario grants a single ability `open` to every player; its handler
// uses the caller's z-position to pick the nearest closed gate to drop.
