...
- The `trigger` field controls when `tick` is called: `'on-player-move'` fires after every `processMove` for any human player; `{ period: number }` fires every `period` ms of simulated time (measured in server ticks, not wall-clock).
