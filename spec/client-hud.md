...
- **Rule popup**: full-screen modal overlay showing one or more rule cards stacked vertically. Each card has a small category label (`RULE`, `COMMAND`, or `FACT`) above the rule text. The player dismisses the popup by tapping or clicking anywhere on the screen, or by pressing any key. At most one rule popup is visible at a time. Does not appear after the local player is eliminated.
